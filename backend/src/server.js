...
app.get('/api/auth/me',auth,asyncRoute(async(req,res)=>{const [[u]]=await pool.query('SELECT id,name,email,phone,role,designation,university,blood_group,photo,must_change_password FROM users WHERE id=?',[req.user.id]);res.json(u)}));
...
app.get('/api/me/certificate',auth,asyncRoute(async(req,res)=>{
  ...
  const [[cert]]=await pool.query('SELECT cert.* FROM certificates cert WHERE cert.participant_id=? AND cert.certificate_url IS NOT NULL ORDER BY cert.id DESC LIMIT 1',[p.id]);
  if(!cert) return res.json(null);

  const [fb]=await pool.query('SELECT id, rating, content_rating, speaker_rating, comment FROM feedback WHERE participant_id=? LIMIT 1',[p.id]);
  const feedbackSubmitted = fb.length > 0;

  res.json({
    ...cert,
    feedback_submitted: feedbackSubmitted,
    feedback: feedbackSubmitted ? fb[0] : null
  });
}));
