...
class ConferenceApp extends StatelessWidget{const ConferenceApp({super.key});@override Widget build(BuildContext c)=>MaterialApp(debugShowCheckedModeBanner:false,title:conferenceName,theme:ThemeData(useMaterial3:true,scaffoldBackgroundColor:cream,colorScheme:ColorScheme.fromSeed(seedColor:maroon),fontFamily:'Arial'),home:const AuthGate());}
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        leading: CircleAvatar(
                          ...
                          child: const Icon(Icons.event, color: maroon),
                        ),
                        title: Text(
                          '${x['title'] ?? ''}',
...
class SpeakersScreen extends StatelessWidget{const SpeakersScreen({super.key});@override Widget build(BuildContext c)=>SimplePage(title:'Speakers',future:ApiService.get('/speakers?conferenceId=1'),builder:(x)=>ListTile(leading:CircleAvatar(backgroundImage:x['photo']!=null?NetworkImage(x['photo']):null,child:x['photo']==null?const Icon(Icons.person):null),title:Text(x['name']??'',style:const TextStyle(fontWeight:FontWeight.w800)),subtitle:Text('${x['designation']??''}\n${x['organization']??''}'),isThreeLine:true));}
...
class EmergencyScreen extends StatelessWidget{const EmergencyScreen({super.key});@override Widget build(BuildContext c)=>Scaffold(appBar:AppBar(title:const Text('Emergency & Help')),body:ListView(padding:const EdgeInsets.all(18),children:[CardButton(icon:Icons.support_agent,title:'Conference Help Desk',subtitle:'1800123456',onTap:()=>launchUrl(Uri.parse('tel:1800123456'))),CardButton(icon:Icons.local_hospital,title:'Medical Emergency',subtitle:'Call 108',onTap:()=>launchUrl(Uri.parse('tel:108'))),CardButton(icon:Icons.security,title:'Security',subtitle:'Call 100',onTap:()=>launchUrl(Uri.parse('tel:100'))),CardButton(icon:Icons.location_on,title:'Venue Map',subtitle:'Open in Google Maps',onTap:()=>launchUrl(Uri.parse(mapUrl),mode:LaunchMode.externalApplication))]));}
