...
function Login({onLogin}){const[e,setE]=useState('admin@conference.local'),[p,setP]=useState('Admin@123'),[busy,setBusy]=useState(false);const submit=async x=>{x.preventDefault();setBusy(true);try{const d=await req('/auth/login',{method:'POST',body:JSON.stringify({email:e,password:p})});localStorage.setItem('token',d.token);onLogin()}catch(err){alert(err.message)}finally{setBusy(false)}};return <div className="login"><form onSubmit={submit}><div className="brand">DPU</div><h1>Conference Control Room</h1><p>Manage live conference content, logistics, and mobile app data from MySQL.</p><input value={e} onChange={x=>setE(x.target.value)} placeholder="Email"/><input type="password" value={p} onChange={x=>setP(x.target.value)} placeholder="Password"/><button disabled={busy}>{busy?'Signing in...':'Sign in'}</button><small>Demo: admin@conference.local / Admin@123</small></form></div>}

const menu=[
  {title:'Dashboard',icon:LayoutDashboard},
  {title:'Conference',icon:Building2,children:['Conference Details','Venue & Location','Branding','Conference Settings']},
  {title:'Participants',icon:Users,children:['All Participants','Add Participant','Import Participants','Registration','QR Codes']},
  {title:'Speakers',icon:Users,children:['All Speakers','Add Speaker']},
  {title:'Schedule',icon:CalendarDays,children:['Sessions','Tracks','Halls','Session Speakers']},
  {title:'Accommodation',icon:Hotel,children:['Hotels','Rooms','Room Allocation']},
  {title:'Transport',icon:Bus,children:['Vehicles','Drivers','Pickup Points','Transport Assignments']},
  {title:'Attendance',icon:CheckCircle,children:['Live Attendance','QR Scanner','Attendance Reports']},
  {title:'Notices',icon:Bell,children:['Notices','Push Notifications']},
  {title:'Gallery',icon:Image,children:['Albums','Photos','Photo Moderation']},
  {title:'Meals',icon:CalendarDays,children:['Meal Schedule','Meal Attendance']},
  {title:'Duties',icon:Shield,children:['Duty Roster','Assignments']},
  {title:'Certificates',icon:FileCheck,children:['Templates','Generate Certificates','Certificate Verification']},
  {title:'Chat',icon:Bell,children:['Conversations','Messages']},
  {title:'Reports',icon:FileCheck,children:['Participants','Attendance','Accommodation','Transport','Meals','Certificates']},
  {title:'Admin Users',icon:Lock,children:['Users','Roles','Permissions']},
...
function App(){
  ...
  const[open,setOpen]=useState({Conference:true});
...
function renderPage(tab,conference,setConference,notify){
  ...
  if(['Conference Details','Venue & Location','Branding','Conference Settings'].includes(tab))return <ConferenceModule tab={tab} conference={conference} setConference={setConference} notify={notify}/>;
  if(tab==='All Participants'||tab==='Add Participant'||tab==='Participants')return <Participants tab={tab} notify={notify}/>;
  if(tab==='All Speakers'||tab==='Add Speaker'||tab==='Speakers')return <Speakers tab={tab} notify={notify}/>;
  if(tab==='Sessions'||tab==='Tracks'||tab==='Halls'||tab==='Schedule')return <Schedule tab={tab} notify={notify}/>;
  if(tab==='Hotels'||tab==='Rooms'||tab==='Accommodation')return <Hotels tab={tab} notify={notify}/>;
  if(tab==='Room Allocation')return <RoomAllocation notify={notify}/>;
  if(tab==='Vehicles'||tab==='Drivers'||tab==='Transport')return <Transport tab={tab} notify={notify}/>;
  if(tab==='Transport Assignments')return <TransportAssignments notify={notify}/>;
  if(tab==='Albums'||tab==='Photos'||tab==='Gallery')return <Gallery notify={notify}/>;
  if(tab==='Meal Schedule'||tab==='Meals')return <Meals notify={notify}/>;
  if(tab==='Duty Roster'||tab==='Duties')return <Duties notify={notify}/>;
  if(tab==='Templates'||tab==='Generate Certificates'||tab==='Certificates')return <Certificates notify={notify}/>;
  if(tab==='Reports')return <Reports notify={notify}/>;
  if(tab==='Conversations'||tab==='Messages'||tab==='Chat')return <Chat notify={notify}/>;
  if(tab==='Users'||tab==='Roles'||tab==='Permissions'||tab==='Admin Users')return <AdminUsers notify={notify}/>;
  if(tab==='Live Attendance'||tab==='Attendance')return <Attendance notify={notify}/>;
  if(tab==='Sessions'||tab==='Schedule')return <Schedule/>;
...
function Meals({notify}){
  ...
  return <div className="panel">
    <div className="pagehead"><div><h3>Meal Schedule</h3><p>Manage conference catering timeline and menu.</p></div><div className="actions"><button onClick={()=>setShowAdd(true)}>Schedule Meal</button></div></div>
...
function MealModal({onSave,onClose}){
  const[v,set]=formState({});
  return <div className="modal-overlay"><div className="modal"><div className="modal-header"><h3>Schedule Meal</h3></div><div className="modal-body"><div className="formgrid">
...
function Duties({notify}){
  ...
  return <div className="panel">
    <div className="pagehead"><div><h3>Duty Roster</h3><p>Manage volunteer and staff assignments.</p></div><div className="actions"><button onClick={()=>setShowAdd(true)}>Create Duty</button><button onClick={()=>setShowAssign(true)}>Assign Staff</button></div></div>
    <div className="split">
      <div className="panel"><h4>Scheduled Duties</h4>{d.map(x=><div className="rowcard" key={x.id}><div><b>{x.title}</b><p>{x.location}<br/>{toInputDate(x.duty_date)} {x.start_time}-{x.end_time}</p></div></div>)}</div>
      <div className="panel"><h4>Staff Assignments</h4>{assignments.map(x=><div className="rowcard" key={x.id}><div><b>{x.user_name}</b><p>{x.duty_title} - {x.status}</p></div></div>)}</div>
    </div>
...
function DutyModal({onSave,onClose}){
  const[v,set]=formState({});
  return <div className="modal-overlay"><div className="modal"><div className="modal-header"><h3>Create Duty</h3></div><div className="modal-body"><div className="formgrid">
    <Field label="Title" value={v.title} onChange={x=>set('title',x)}/>
    <Field label="Location" value={v.location} onChange={x=>set('location',x)}/>
...
function DutyAssignmentModal({duties,users,onSave,onClose}){
  const[v,set]=formState({});
  return <div className="modal-overlay"><div className="modal"><div className="modal-header"><h3>Assign Staff</h3></div><div className="modal-body"><div className="formgrid">
...
function Certificates({notify}){
  ...
  useEffect(()=>{load()},[]);
  return <div className="panel">
    <div className="pagehead"><div><h3>Certificates</h3><p>Manage and issue conference certificates.</p></div><div className="actions"><button onClick={()=>setShowAdd(true)}>Issue Certificate</button></div></div>
    <table>
      <thead><tr><th>Participant</th><th>Certificate No</th><th>Issued At</th><th>Actions</th></tr></thead>
      <tbody>
        {d.map(x=><tr key={x.id}>
          <td><b>{x.participant_name}</b><br/><small>{x.registration_no}</small></td>
          <td>{x.certificate_no}</td>
          <td>{new Date(x.issued_at).toLocaleString()}</td>
          <td><button onClick={()=>window.open(API.replace('/api','/verify/')+x.certificate_no)}>Verify</button></td>
        </tr>)}
      </tbody>
    </table>
    {showAdd && <IssueModal participants={participants} onSave={async(v)=>{await req(`/admin/certificates/${v.participant_id}/issue`,{method:'POST',body:v}); setShowAdd(false); load(); notify('Certificate issued');}} onClose={()=>setShowAdd(false)}/>}
  </div>
}

function IssueModal({participants,onSave,onClose}){
  const[v,set]=formState({});
  return <div className="modal-overlay"><div className="modal"><div className="modal-header"><h3>Issue Certificate</h3></div><div className="modal-body"><div className="formgrid">
    <SelectField label="Participant" value={v.participant_id} onChange={x=>set('participant_id',x)} options={['',...participants.map(p=>({value:p.id,label:p.name}))].map(o=>typeof o==='string'?o:o.label)}/>
    <Field label="Certificate No (Optional)" value={v.certificateNo} onChange={x=>set('certificateNo',x)}/>
  </div></div><div className="modal-footer"><button onClick={onClose}>Cancel</button><button className="primary" onClick={()=>onSave({...v, participant_id: participants.find(p=>p.name===v.participant_id)?.id})}>Issue</button></div></div></div>
}

function Reports({notify}){
  const[type,setType]=useState('participants'),[data,setData]=useState([]),[busy,setBusy]=useState(false);
  const load=async()=>{setBusy(true); try{const r=await req(`/admin/reports/${type}`);setData(r)}finally{setBusy(false)}};
  useEffect(()=>{load()},[type]);
  return <div className="panel">
    <div className="pagehead"><div><h3>Reports</h3><p>Generate and export conference data.</p></div><div className="actions"><button onClick={()=>alert('Exporting to Excel...')}>Export Excel</button></div></div>
    <div className="toolbar"><SelectField label="Report Type" value={type} onChange={setType} options={['participants','attendance']}/></div>
    <table>
...
function Chat({notify}){
  ...
  const load=async()=>{const r=await req('/admin/conversations?conferenceId=1');setConvs(r)};
  useEffect(()=>{load()},[]);
  const loadMsgs=async(id)=>{const r=await req(`/conversations/${id}/messages`);setMsgs(r);setActive(id)};
  ...
  return <div className="panel chat-panel"><div className="split">
    <div className="panel list"><h4>Conversations</h4>{convs.map(c=><div key={c.id} className={`rowcard ${active===c.id?'active':''}`} onClick={()=>loadMsgs(c.id)}><b>{c.title}</b><p>{c.last_message||'No messages'}</p></div>)}</div>
    <div className="panel msgs">
      <h4>Messages</h4>
      <div className="msg-list">{msgs.map(m=><div key={m.id} className="msg"><b>{m.sender_name}:</b> {m.body}</div>)}</div>
      <div className="input"><input value={text} onChange={e=>setText(e.target.value)}/><button onClick={send}>Send</button></div>
    </div>
  </div></div>
}
...
function AdminUsers({notify}){
  ...
  useEffect(()=>{load()},[]);
  return <div className="panel">
    <div className="pagehead"><div><h3>Admin Users</h3><p>Manage administrative access and audit logs.</p></div><div className="actions"><button onClick={()=>setShowAdd(true)}>Create User</button></div></div>
    <div className="split">
      <div className="panel"><h4>Users</h4><table><thead><tr><th>Name</th><th>Role</th></tr></thead><tbody>{d.map(x=><tr key={x.id}><td><b>{x.name}</b><br/>{x.email}</td><td>{x.role}</td></tr>)}</tbody></table></div>
      <div className="panel"><h4>Audit Logs</h4><div className="log-list">{logs.map(x=><div key={x.id} className="log-item"><small>{new Date(x.created_at).toLocaleString()}</small><p><b>{x.admin_name}</b>: {x.action} on {x.entity}</p></div>)}</div></div>
    </div>
    {showAdd && <AdminUserModal onSave={async(v)=>{await req('/admin/users',{method:'POST',body:v}); setShowAdd(false); load(); notify('Admin created');}} onClose={()=>setShowAdd(false)}/>}
  </div>
}

function AdminUserModal({onSave,onClose}){
  const[v,set]=formState({role:'ADMIN'});
  return <div className="modal-overlay"><div className="modal"><div className="modal-header"><h3>Create Admin</h3></div><div className="modal-body"><div className="formgrid">
    ...
    <SelectField label="Role" value={v.role} onChange={x=>set('role',x)} options={['ADMIN','EVENT_MANAGER','TRANSPORT_ADMIN','VOLUNTEER']}/>
  </div></div><div className="modal-footer"><button onClick={onClose}>Cancel</button><button className="primary" onClick={()=>onSave(v)}>Create</button></div></div></div>
}
